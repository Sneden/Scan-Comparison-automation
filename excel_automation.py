...
with open('~/Downloads/XXX-PUB-External_All.csv', 'w') as f:
    f.write(response)


##Move file to workdir
original = r'~/Downloads/XXX-PUB-External_All.csv'
target = r'~/Documents/Workdir/XXX-PUB-External_All.csv'
shutil.move(original, target)


##Delete unrequired columns of XXX-PUB-External_All.csv file
df = pd.read_csv('~/Documents/Workdir/XXX-PUB-External_All.csv', low_memory=False)
...
remove_dup_ip.to_csv('~/Documents/Workdir/XXX-PUB-External_All.csv', index = False)


##Append new + old and save in .csv file (old) then Delete repeated ip's from old file
print("\n\nSyncing in process ~ Removing duplicates\n\n")
rf = pd.read_csv('~/Documents/Workdir/XXX-PUB-External_All.csv')
tf = pd.read_csv('~/Documents/Workdir/original.csv')
frames = [tf, rf]
...
print(mids1)
print("\n")
mids1.to_csv('~/Documents/Workdir/new_ips.csv', index = False)
dup.to_csv('~/Documents/Workdir/original.csv', index = False)


##delete new file as already synced from workdir and downloads folder
os.remove('~/Documents/Workdir/XXX-PUB-External_All.csv')
print("\n\nFile is synced successfully\n\n")


##Add synced IP's & ports to research file && remove duplicate values already researched
cf = pd.read_csv('~/Documents/Workdir/original.csv')
